# AGENTS.md — Heater MicroDrop plugin
...
- `heater_controller` — backend: `PeripheralDeviceControllerBase` subclass,
  serial proxy, `on_<topic>_request` handlers, `consts.py` with the topics
  and `ACTOR_TOPIC_DICT`.
- `heater_controls_ui` — dock pane, status-bar icon, `BaseMessageHandler`
  subclass with `_on_<topic>_triggered` handlers.
- `heater_protocol_controls` — protocol-tree column(s).
