...
class SensorConfigController(Controller):
    ...
    def save_and_push(self, info=None):
        ...
        if (
            confirm(
                message="Write this configuration to the board and reboot it?",
                informative="The heater will disconnect briefly while it reboots.",
                title="Save && push to board",
            )
            != YES
        ):
            return
        ...
        information(
            ...
            informative="It will reboot and reconnect shortly.",
            title="Pushing config",
        )
    ...
    def _validated_config(model):
        ...
        assignments = {
            r.heater: split_sensor_names(r.sensors) for r in model.heater_assignments
        }
        try:
            HeaterConfigEdit(
            ...
            error(
                message="The configuration can't be saved:",
                informative=details,
