    ...
    VGroup(
        HGroup(
            ...
            UItem(
                ...
                editor=IconButtonEditor(
                    glyph=ICON_HOME,
                    tooltip="Back to the current experiment's heater logs (newest log)",
                ),
            ),
            UItem(
                "handler.refresh_button",
                editor=IconButtonEditor(
                    glyph=ICON_REFRESH,
        ...
        UItem("sensor_series", editor=CustomEditor(log_canvas_factory), springy=True),
    ),
    resizable=True,
)
...
class HeaterLogViewerController(Controller):
    ...
    def _refresh_logs(self, event=None):
        ...
        if directory is not None and directory.is_dir():
            logs = sorted(
                directory.glob(LOG_FILE_GLOB), key=lambda path: path.stat().st_mtime
            )
    ...
    def _show_saved_log(self, event):
        ...
        if str(saved_path.parent) != self.model.directory:
            ...
            self._refresh_logs()
        if (
            saved_path.name in self.model.log_names
